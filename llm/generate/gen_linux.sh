...
if [ -d "${ONEAPI_ROOT}" ]; then
    ...
    CC=icx
    CMAKE_DEFS="${COMMON_CMAKE_DEFS} ${CMAKE_DEFS} -DCMAKE_C_COMPILER=icx -DCMAKE_CXX_COMPILER=icpx -DLLAMA_SYCL=ON -DLLAMA_SYCL_F16=ON -DLLAMA_F16C=ON"
...
#    echo "OneAPI libraries detected - building dynamic OneAPI library"
#    init_vars
#    CMAKE_DEFS="${COMMON_CMAKE_DEFS} ${CMAKE_DEFS} -DLLAMA_AVX512=on -DLLAMA_AVX2=on -DLLAMA_F16C=on -DLLAMA_FMA=on -DLLAMA_SYCL=ON -DCMAKE_C_COMPILER=icx -DCMAKE_CXX_COMPILER=icpx"
#    BUILD_DIR="${LLAMACPP_DIR}/build/linux/${ARCH}/oneapi"
